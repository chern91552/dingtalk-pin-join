 ...
 * 群设置页中的置顶查证任务状态栏。
 */
public final class ForestAuditStatusView
        implements ForestAudit.StatusListener, View.OnAttachStateChangeListener {
    ...
    public ForestAuditStatusView(Activity activity, boolean dark) {
        ...
        stopButton.setOnClickListener(v -> ForestAudit.stop());
        root.addView(stopButton);

        ForestAudit.addStatusListener(this);
    }
    ...
    @Override
    public void onStatusChanged(ForestAudit.StatusSnapshot snapshot) {
        ...
        if (snapshot.status == ForestAudit.STATUS_IDLE) {
            root.setVisibility(View.GONE);
            return;
        }

        root.setVisibility(View.VISIBLE);
        if (snapshot.status == ForestAudit.STATUS_RUNNING) {
            statusDot.setBackground(circle(0xFF1677FF));
            String text = "置顶查证中  " + snapshot.completed + "/" + snapshot.target;
        ...
        int terminalColor = snapshot.status == ForestAudit.STATUS_COMPLETED
                ? 0xFF2E9B57
                : snapshot.status == ForestAudit.STATUS_ERROR ? 0xFFE05252 : 0xFF8A8F98;
    ...
    public void onViewDetachedFromWindow(View view) {
        mainHandler.removeCallbacks(hideResult);
        ForestAudit.removeStatusListener(this);
    }
